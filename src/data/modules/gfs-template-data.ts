...
const M19: GfsTemplate = {
  ...
  subLessons: [
    { id: "sl1", title: "Cloud Computing Concepts",
      body: "**NIST's 5-4-3 definition.**\n- **5 essentials:** on-demand self-service, broad network access, resource pooling, rapid elasticity, measured service.\n- **4 deployment models:** public, private, hybrid, community.\n- **3 service models:** IaaS (you manage OS up), PaaS (you manage app up), SaaS (you manage data + users).\n\n**Shared Responsibility — the single most tested concept.** The provider secures **the cloud** (hardware, hypervisor, physical DC). You secure **what's in it** (data, IAM, config, code). Moving up the stack (IaaS → SaaS) shifts *more* to the provider but never removes your responsibility for identity and data.",
    ...
    { id: "sl2", title: "Cloud Computing Threats",
      body: "CSA's **\"Pandemic Eleven\"** top threats — the ones actually breaking real tenants:\n\n- Weak **IAM** (over-permissive roles, no MFA).\n- Insecure APIs and interfaces.\n- **Misconfiguration** — #1 root cause per Verizon DBIR.\n- Missing cloud security architecture.\n- Insecure software development.\n- Unsecured third-party components.\n- Unpatched system vulnerabilities.\n- Accidental data disclosure.\n- Misconfigured serverless.\n- APTs targeting cloud tenants.\n- Data exfiltration from cloud storage.\n\n**Add to the list:** metadata SSRF (Capital One 2019), leaked keys in GitHub, and SSO takeover via misconfigured trust.",
    ...
    { id: "sl3", title: "Cloud Hacking",
      body: "Cloud attacks follow the same kill chain across every provider:\n\n1. **External recon** — enumerate S3/GCS/Blob buckets, mine certificate-transparency logs, scan GitHub for leaked keys.\n2. **Initial access** — leaked keys, phished SSO, exposed Jenkins or Kubernetes dashboards.\n3. **Enumerate** — Pacu (AWS), MicroBurst/ROADrecon (Azure), GCPTokenReuse (GCP) list every permission the compromised identity has.\n4. **Escalate** — chain `iam:PassRole`, inherit subscription-owner, or ride a Managed Identity.\n5. **Persist** — new IAM user, backdoor Lambda/Function, cross-account trust.\n6. **Exfiltrate** — `aws s3 sync` to an attacker-controlled bucket.",
    ...
    { id: "sl4", title: "AWS Hacking",
      body: "**Enumerate.** `aws sts get-caller-identity`, then ScoutSuite / Prowler / Pacu modules (`iam__enum_permissions`, `iam__privesc_scan`).\n\n**The recurring findings.**\n- IAM policies with `*` on Action or Resource.\n- **IMDSv1** still on — any SSRF steals the workload role (Capital One).\n- Public S3 buckets because account-level Block Public Access is off.\n- Unencrypted EBS snapshots or public AMIs.\n- Secrets sitting in Lambda environment variables.\n- CloudTrail disabled in secondary regions.\n- Root user with active access keys.\n\n**Escalation.** Rhino Security documented **21+ IAM privesc paths** (e.g. `CreatePolicyVersion`, `PassRole + CreateFunction`). Know them.",
    ...
    { id: "sl5", title: "Microsoft Azure Hacking",
      body: "**Identity is everything on Azure.** Attacks centre on **Entra ID** (formerly Azure AD):\n\n- **Password spray** against `login.microsoftonline.com`.\n- **Device-code phishing** — legit MS URL, victim types your code.\n- **Consent phishing** — victim grants a malicious app permission to their mailbox.\n- **PRT (Primary Refresh Token) theft** from a domain-joined workstation.\n\n**Toolbelt.** MicroBurst, ROADrecon, AzureHound, Stormspotter, PowerZure.\n\n**Recurring findings.** Legacy auth still on, too many Global Admins, no Conditional Access on privileged roles, Managed Identity with Contributor on the whole subscription, storage accounts with `allowBlobPublicAccess=true`, Key Vaults without soft-delete.",
    ...
    { id: "sl6", title: "Google Cloud Hacking",
      body: "**Enumerate.** `gcloud auth list`, GCPBucketBrute for storage, IAM Recommender for over-privilege.\n\n**GCP-specific attack paths.**\n- **Service-account impersonation** via `iam.serviceAccounts.getAccessToken`.\n- `actAs` chains that let one SA pretend to be another.\n- Deploy a Cloud Function → abuse the workload identity.\n- Hit the metadata server at `169.254.169.254` for the compute SA's token.\n\n**Recurring findings.** Default compute SA still has **Editor** role (huge blast radius), public GCS buckets, Cloud SQL with a public IP and no Auth Proxy, service-account keys older than 90 days.\n\n**Modern controls.** IAM **Deny** policies, Org Policy, VPC Service Controls perimeters.",
    ...
    { id: "sl7", title: "Container Hacking",
      body: "**Where containers break.**\n- Vulnerable base image (scan with Trivy / Grype).\n- Image pulled from an untrusted registry.\n- Pod with `--privileged` or `hostPath` mounts → trivial node escape.\n- Exposed Docker / containerd socket.\n- Kubelet on `:10250` without auth.\n- Kubernetes API server allowing anonymous access.\n- ServiceAccount token auto-mounted with cluster-admin.\n\n**Attacker toolbelt.** kube-hunter, kubescape, kubectl-who-can, botb (Break Out The Box), CDK, Peirates.\n\n**Runtime detection.** Falco, Sysdig Secure, Tetragon spot syscall anomalies live.",
    ...
    { id: "sl8", title: "Cloud Security",
      body: "The defence baseline, six categories:\n\n1. **Identity** — federate to the corporate IdP, ban static keys, enforce MFA/FIDO2, use PIM/JIT for privileged roles.\n2. **Data** — encrypt at rest with customer-managed KMS keys, Block Public Access account-wide.\n3. **Network** — private endpoints, VPC Service Controls, egress firewall.\n4. **Detection** — CloudTrail / Defender / Cloud Audit Logs on in every region, immutable.\n5. **CI/CD** — OIDC federation (no long-lived keys), signed artefacts (cosign), SLSA L3+.\n6. **IR** — cloud-native runbook, immutable snapshots, out-of-band admin access.\n\n**Standards / tooling.** CIS Benchmarks, CSA CCM, NIST 800-53. CSPM (Wiz, Prisma, Defender for Cloud), CIEM (Ermetic), and CNAPP unify these.",
...
const M20: GfsTemplate = {
  ...
  subLessons: [
    { id: "sl1", title: "Cryptography Concepts & Algorithms",
      body: "**What crypto delivers.** Four properties: confidentiality, integrity, authentication, non-repudiation.\n\n**Symmetric (one shared key).** Fast. AES-128/192/256, ChaCha20. **The mode matters more than the key size:**\n- **ECB** — leaks patterns, never use.\n- **CBC** — needs random IV + separate MAC.\n- **CTR** — needs unique nonces.\n- **AEAD** (GCM, ChaCha20-Poly1305) — confidentiality + integrity in one primitive. **Default choice today.**\n\n**Asymmetric (key pair).** Slower, used for signing and key exchange. RSA-2048+, ECDSA P-256/P-384, Ed25519, X25519.\n\n**Hashes.** SHA-256, SHA-3, BLAKE2/3 are safe. **MD5 and SHA-1 are broken.** HMAC = hash + key for integrity.\n\n**Forward secrecy.** Use ephemeral key exchange (**ECDHE**) so yesterday's traffic stays safe if today's key leaks.",
    ...
    { id: "sl2", title: "Applications of Cryptography",
      body: "Where you'll actually meet crypto:\n\n- **Transport:** TLS 1.3, IPsec, WireGuard, SSH.\n- **Email:** S/MIME, PGP/GPG.\n- **Disks / files:** BitLocker, FileVault, LUKS; DB TDE.\n- **Object storage:** envelope encryption + KMS (a per-object data key wrapped by a root KMS key).\n- **Code signing:** Authenticode (Windows), cosign / Sigstore, GPG.\n- **Passwords:** hash with **Argon2id**, scrypt, or bcrypt — *never* plain SHA-256.\n\n**PKI = how trust scales.** X.509 certificates, Certificate Authorities, OCSP stapling, Certificate Transparency logs, ACME automation (Let's Encrypt).",
    ...
    { id: "sl3", title: "Cryptanalysis & Crypto Attacks",
      body: "Attacks come in three flavours:\n\n**1. Against the maths.** Frequency analysis (classical), known-plaintext, chosen-plaintext/ciphertext, differential and linear cryptanalysis, birthday attacks on hashes.\n\n**2. Against the protocol / implementation.** Padding-oracle (**POODLE**, **Lucky13**), **BEAST**, compression side-channel (**CRIME**, **BREACH**), **DROWN** (SSLv2 cross-protocol), Bleichenbacher on RSA PKCS#1v1.5, **ROCA** (Infineon RSA keygen). **Nonce reuse** in GCM or ECDSA is catastrophic — it recovered the Sony PS3 signing key.\n\n**3. Against the hardware.** Side-channels: timing, power (SPA/DPA), EM, cache (Spectre/Meltdown, Flush+Reload).\n\n**Downgrade attacks.** FREAK, Logjam, POODLE — trick both ends into using weaker crypto.\n\n**Quantum.** Shor's algorithm breaks RSA/ECC when large quantum computers exist. Grover halves symmetric strength → **AES-256 stays safe**.",
    ...
    { id: "sl4", title: "Crypto Attack Countermeasures",
      body: "**Set an algorithm floor.** TLS 1.2+ with AEAD suites, ECDHE for forward secrecy, HSTS with preload, OCSP stapling. **Ban** RC4, 3DES, MD5, SHA-1, RSA-1024.\n\n**Use vetted libraries.** libsodium, BoringSSL, Bouncy Castle. **Never roll your own crypto.**\n\n**Manage keys properly.**\n- Root keys in HSM / KMS.\n- Per-service data keys via envelope encryption.\n- Automated rotation + anomalous-decrypt alerts.\n- Quorum (M-of-N) for root-key administration.\n\n**Code-signing keys.** Belong in an HSM — or better, ephemeral CI keys via Sigstore/Fulcio with SLSA provenance.\n\n**Start crypto-agility today.** Wrap algorithm choice behind an interface so you can swap it. Pilot the NIST **post-quantum** finalists — **ML-KEM (Kyber)**, **ML-DSA (Dilithium)**, **SLH-DSA (SPHINCS+)** — on long-lifetime data. \"Harvest now, decrypt later\" is already happening.",
