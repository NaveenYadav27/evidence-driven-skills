...
const M18: GfsTemplate = {
  ...
  subLessons: [
    { id: "sl1", title: "IoT Concepts and Attacks",
      body: "IoT = physical devices with sensors/actuators + network + cloud back-end + mobile companion app. Architecture layers: Perception (sensors), Network (Zigbee/Z-Wave/BLE/LoRa/6LoWPAN/Wi-Fi/cellular), Middleware (MQTT/CoAP/AMQP), Application (dashboards, APIs), and Business. Common attacks: default credentials (Mirai), firmware backdoors, weak MQTT auth, insecure OTA updates, side-channel via UART/JTAG, replay of RF captures, and cloud-API takeover through the companion app. OWASP IoT Top 10 codifies these.",
    ...
    { id: "sl2", title: "IoT Hacking Methodology",
      body: "EC-Council flow: (1) Information Gathering — Shodan, Censys, FCC ID lookups, teardown photos; (2) Vulnerability Scanning — Nmap, Nessus, Firmalyzer; (3) Launch Attack — replay RF with HackRF/RTL-SDR, MQTT broker enumeration, BLE sniff with Ubertooth, UART shell drop; (4) Gain Remote Access — exploit CVE, default creds, or backdoor account; (5) Maintain Access — flash modified firmware, enrol into attacker C2. Companion-app review overlaps with M17 mobile methodology.",
    ...
    { id: "sl3", title: "IoT Attack Countermeasures",
      body: "Buy-side controls: procurement checklist requiring signed firmware, unique per-device credentials, disabled debug interfaces, and a supported patch lifecycle. Deploy-side: dedicated IoT VLAN with default-deny egress, DHCP fingerprinting inventory, MDNS/UPnP disabled, and passive NDR (Armis, Claroty xDome) baselining behaviour. NIST IR 8259, ETSI EN 303 645, and the EU Cyber Resilience Act codify the baselines. For consumer IoT deployed in branches, treat every device as hostile until segmented.",
    ...
    { id: "sl4", title: "OT Concepts and Attacks",
      body: "OT (Operational Technology) = ICS/SCADA/DCS/PLC running physical processes: HVAC, building management, factory floors, power/water. Purdue Reference Model splits into Levels 0–5 with an IT/OT DMZ (Level 3.5). Priorities inverted from IT: Availability > Integrity > Confidentiality. Landmark incidents: Stuxnet (2010, centrifuges), Ukraine grid (2015/2016), TRITON/TRISIS (2017, safety systems), Colonial Pipeline (2021), Oldsmar water (2021). Attack goals: process disruption, safety compromise, ransomware for downtime leverage.",
    ...
    { id: "sl5", title: "OT Hacking Methodology",
      body: "OT assessment is passive-first and strictly ROE-governed — active fuzzing has killed people. Sequence: (1) Passive collection via SPAN/TAP with Wireshark + Zeek ICS parsers; (2) Asset inventory from broadcast/multicast; (3) Protocol analysis for Modbus function codes 05/06/15/16 (write coils/registers) which are unauthenticated; (4) Segmentation validation — attempt one-way probe from IT to OT and confirm block; (5) HMI web-app review under IEC 62443 and ISA-99. Any active step requires facilities director sign-off and a maintenance window.",
    ...
    { id: "sl6", title: "OT Attack Countermeasures",
      body: "IEC 62443 zones and conduits are the baseline: define trust zones, enforce conduits with firewalls or unidirectional gateways (Waterfall, OwlCyberDefense) for IT→OT telemetry. Deploy OT-aware NDR (Dragos, Nozomi, Claroty) baselining flows per device and alerting on new talkers or unexpected function codes. Physically isolate safety instrumented systems (SIS) from BPCS. Patch during scheduled outages only; where patching is impossible, apply virtual patching with an ICS-aware IPS. Incident response requires a joint IT/OT playbook with facilities engineers on-call.",
