...
const M16: GfsTemplate = {
  ...
  subLessons: [
    { id: "sl1", title: "Wireless Fundamentals & Standards",
      body: "802.11a/b/g/n/ac/ax across 2.4/5/6 GHz. Association vs authentication vs association-response frames. Management frames are unencrypted pre-WPA3 — the root of most Layer-2 attacks.",
      bullets: ["Beacon / Probe / Auth / Assoc frames", "Managed vs Monitor vs Master modes", "Channels, RSSI, BSSID vs ESSID"] },
    { id: "sl2", title: "WPA2-PSK & the 4-Way Handshake",
      body: "PSK → PMK → PTK via a 4-way EAPOL exchange. Capturing the handshake enables offline dictionary/brute-force. PMKID (RSN IE) removes the need for a client to be present.",
      bullets: ["EAPOL M1–M4 messages", "PMKID single-frame capture", "hashcat mode 22000"] },
    { id: "sl3", title: "WPA2-Enterprise / 802.1X / EAP",
      body: "RADIUS-backed auth. EAP-TLS (mutual cert) is the gold standard; PEAP-MSCHAPv2 is offline-crackable and must be banned in regulated environments.",
      bullets: ["EAP-TLS vs PEAP vs TTLS", "RADIUS server-cert pinning via MDM", "Per-device client certificates"] },
    { id: "sl4", title: "WPA3-SAE, Transition Mode & Dragonblood",
      body: "SAE (Simultaneous Authentication of Equals) replaces PSK's offline-crackable exchange. Transition mode preserves WPA2 compatibility and creates a downgrade path.",
      bullets: ["SAE handshake vs 4-way", "Transition-mode downgrade risk", "Dragonblood side-channel patches"] },
    { id: "sl5", title: "Rogue APs, Evil Twins & Karma",
      body: "Any unmanaged AP mimicking a corporate SSID is a rogue. Karma-class devices respond to client probe requests and capture EAP exchanges wherever employees roam.",
      bullets: ["WIPS auto-containment", "MDM: disable auto-connect", "Physical RF sweeps"] },
  ],
  diagram: {
...
const M17: GfsTemplate = {
  ...
  subLessons: [
    { id: "sl1", title: "Mobile Threat Model & Attack Surface",
      body: "The device is untrusted. Attack surface = store, IPC, storage, transport, runtime, deep-links, backup. Every leaked artefact is a step towards account takeover.",
      bullets: ["OWASP MASVS levels", "Static + dynamic + runtime", "Backend shared with web app"] },
    { id: "sl2", title: "Android Static Analysis",
      body: "AndroidManifest.xml is the trust boundary. Exported components without permission gates are free authenticated actions from any other app on the device.",
      bullets: ["apktool / jadx / MobSF", "android:exported / permissions", "allowBackup, debuggable"] },
    { id: "sl3", title: "iOS Runtime & Keychain",
      body: "Info.plist entitlements, ATS, Keychain access classes, URL scheme handlers. Frida/Objection bypass most Jailbreak checks in one line — layer with App Attest.",
      bullets: ["ATS strict", "Keychain kSecAttrAccessible*", "Universal Links vs schemes"] },
    { id: "sl4", title: "Transport Security & Pinning",
      body: "TLS alone is not enough. Certificate pinning binds the client to a known chain; bypass telemetry surfaces attackers who succeed anyway.",
      bullets: ["network-security-config", "Pin intermediate, not leaf", "Server-side attestation signals"] },
    { id: "sl5", title: "Deep Links, IPC & JS Bridges",
      body: "Deep links, exported providers, and WebView JS bridges are first-class attack surface. Any sensitive action must re-authenticate.",
      bullets: ["App Links / Universal Links + asset links", "Signature-level intent permissions", "JS bridge allow-list"] },
  ],
  diagram: {
