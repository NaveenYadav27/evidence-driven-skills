...
import {
  M03_SCENARIO, M03_WORKFLOW, M03_ANALYST_FRAMEWORK, M03_GUIDED,
  M03_INCIDENTS, M03_DELIVERABLES, M03_AI_ACTIONS,
} from "@/data/modules/m03";
import { TermText, Term } from "@/components/modules/m02/Term";

/* ---------- Professional Scenario ---------- */
export function ProfessionalScenario() {
  const rows: [string, string][] = [
    ["Client", M03_SCENARIO.client],
    ["Assessment", M03_SCENARIO.assessment],
    ["Scope", M03_SCENARIO.scope],
    ["Available Info", M03_SCENARIO.available.join(" · ")],
    ["Expected Outcome", M03_SCENARIO.outcome],
  ];
  return (
    <section className="panel panel-accent p-6">
      ...
      <div className="mt-4 border-l-2 border-[var(--cyan)]/40 pl-3 text-xs text-muted-foreground leading-relaxed">
        <span className="text-[var(--cyan)] font-semibold">Why this matters · </span><TermText>{M03_SCENARIO.why}</TermText>
      </div>
    </section>
  );
}
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm uppercase tracking-wider font-semibold">Scanning Workflow</h3>
      </div>
  ...
  return (
    <section>
      ...
      <p className="text-xs text-muted-foreground mb-3">
        Observation → Finding → Exposure → Attack Opportunity → Business Risk → Recommendation. Repeat for every port, banner, and fingerprint you record.
      </p>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {M03_ANALYST_FRAMEWORK.map((e, i) => (
  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm uppercase tracking-wider font-semibold">Real-World Scanning Incidents</h3>
      </div>
...
export function DeliverableTracker({ storageKey = "m03-deliverables" }: { storageKey?: string }) {
  ...
  const done = M03_DELIVERABLES.filter((d) => checked[d.id]).length;
  ...
  return (
    <section className="panel p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-xs font-mono text-muted-foreground">{done} / {M03_DELIVERABLES.length}</div>
      </div>
      <div className="h-1 w-full rounded bg-secondary/60 overflow-hidden mb-3">
        <div className="h-full bg-[var(--cyan)] transition-all" style={{ width: `${(done / M03_DELIVERABLES.length) * 100}%` }} />
      </div>
      <ul className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
        {M03_DELIVERABLES.map((d) => {
...
export function AiActions() {
  ...
  return (
    <section className="panel p-5">
      ...
      <p className="text-xs text-muted-foreground mb-3">
        Augment your scan results. Each action produces a reasoned analyst output you can paste into your report and adapt.
      </p>
      <div className="flex flex-wrap gap-2 mb-3">
        {M03_AI_ACTIONS.map((a) => (
          <button
            key={a.id}
      ...
      {open && (
        ...
        >
          <TermText>{M03_AI_ACTIONS.find((a) => a.id === open)?.output ?? ""}</TermText>
        </motion.div>
      )}
    </section>
  );
...
/* ---------- Full module enhancement block ---------- */
export function M03ModuleEnhancements() {
  return (
    <div className="space-y-6">
      ...
      <AiActions />
      <RealIncidents />
      <DeliverableTracker storageKey="m03-deliverables-module" />
    </div>
  );
}
